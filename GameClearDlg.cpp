...
CGameClearDlg::CGameClearDlg(int score, int highScore, CWnd* pParent)
    : CDialogEx(IDD_DIALOG_GAMECLEAR, pParent), m_score(score), m_highScore(highScore)
{
}

void CGameClearDlg::DoDataExchange(CDataExchange* pDX)
{
    CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CGameClearDlg, CDialogEx)
    ON_BN_CLICKED(IDC_BUTTON_RESTART, &CGameClearDlg::OnBnClickedRestart)
    ON_WM_ERASEBKGND()
    ON_WM_CTLCOLOR()
    ON_STN_CLICKED(IDC_STATIC_SCORE, &CGameClearDlg::OnStnClickedStaticScore)
    ON_BN_CLICKED(IDC_SCORE_LIST, &CGameClearDlg::OnBnClickedScoreList)
END_MESSAGE_MAP()

BOOL CGameClearDlg::OnInitDialog()
{
    ...
    // 배경 이미지 로드
    m_Background.LoadBitmap(IDB_BACKGROUND_GAMECLEAR);

    return TRUE;
}

BOOL CGameClearDlg::OnEraseBkgnd(CDC* pDC)
{
...
HBRUSH CGameClearDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CGameClearDlg::OnBnClickedRestart()
{
    EndDialog(IDOK);  // OK 리턴으로 메인 다이얼로그에 알려줌
}
void CGameClearDlg::OnStnClickedStaticScore()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}

void CGameClearDlg::OnBnClickedScoreList()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}